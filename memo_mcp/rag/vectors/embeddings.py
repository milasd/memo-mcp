...
class EmbeddingManager:
    ...
    def _load_sentence_transformer(self) -> None:
        ...
        try:
            ...
            self.model = SentenceTransformer(
                self.model_name,
                device=self.device if self.device != "mps" else "cpu"  # SentenceTransformers may not support MPS
            )
    ...
    def embed_texts(self, texts: List[str]) -> List[np.ndarray]:
        ...
        for i, text in enumerate(texts):
            if not text.strip():
                cached_embeddings[i] = np.zeros(self.embedding_dimension, dtype=np.float32)
                continue
    ...
    def _embed_with_huggingface(self, texts: List[str]) -> List[np.ndarray]:
        ...
        try:
            ...
            self.logger.error(f"Failed to generate embeddings with HuggingFace model: {e}")
            raise
