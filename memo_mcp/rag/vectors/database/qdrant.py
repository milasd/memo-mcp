...
class QdrantBackend(VectorDatabase):
    ...
    async def initialize(self) -> None:
        ...
        self.logger.info(f"Initializing Qdrant backend at {self.config.qdrant_host}:{self.config.qdrant_port}")
        ...
        try:
            ...
            raise ConnectionError(f"Cannot connect to Qdrant at {self.config.qdrant_host}:{self.config.qdrant_port}")
    ...
    async def _setup_collection(self) -> None:
        ...
        try:
            ...
            if not collection_exists:
                ...
                if collection_info.config.params.vectors.size != self.config.embedding_dimension:
                    raise ValueError(
    ...
    async def add_documents(
        ...
        for embedding, text, metadata in zip(embeddings, texts, metadatas):
            ...
            payload = {
                ...
                "year": metadata.date_created[:4] if len(metadata.date_created) >= 4 else "",
                "month": metadata.date_created[:7] if len(metadata.date_created) >= 7 else "",
            }
        ...
        self.logger.debug(f"Added {len(points)} documents to Qdrant in {len(range(0, len(points), batch_size))} batches")
    ...
    async def search_with_filter(
        ...
        if month_filter:
            conditions.append(
                FieldCondition(
                    key="month",
                    match=MatchValue(value=month_filter)
                )
            )
    ...
    async def create_payload_index(self, field_name: str, field_type: str = "keyword") -> None:
        ...
        try:
            ...
            self.logger.info(f"Created payload index on field: {field_name} ({field_type})")
    ...
    async def optimize_collection(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Triggered optimization for collection: {self.collection_name}")
            return True
