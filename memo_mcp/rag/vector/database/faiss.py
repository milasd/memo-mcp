...
class FAISSBackend(VectorDatabase):
    ...
    async def remove_document(self, file_path: str) -> bool:
        ...
        indices_to_remove = [
            i for i, meta in enumerate(self.metadatas) if meta.file_path == file_path
        ]
        ...
        new_texts = [
            text for i, text in enumerate(self.texts) if i not in indices_to_remove
        ]
        new_metadatas = [
            meta for i, meta in enumerate(self.metadatas) if i not in indices_to_remove
        ]
        ...
        if new_texts:
            ...
            self.logger.warning(
                f"Document removal requires rebuilding embeddings for {file_path}"
            )
