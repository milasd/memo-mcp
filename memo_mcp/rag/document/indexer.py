...
class DocumentIndexer:
    ...
    def __init__(self, config: RAGConfig, embedding_manager: EmbeddingManager, vector_store: VectorStore):
        self.config = config
    ...
    async def index_documents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Indexing completed in {stats['duration']:.2f}s: "
    ...
    def _discover_memo_files(self) -> List[Path]:
        ...
        if not self.config.data_root.exists():
            self.logger.warning(f"Memo directory does not exist: {self.config.data_root}")
            return memo_files
        ...
        for year_dir in self.config.data_root.iterdir():
            ...
            for month_dir in year_dir.iterdir():
                ...
                for day_file in month_dir.iterdir():
                    if day_file.is_file() and day_file.suffix in self.config.supported_extensions:
                        memo_files.append(day_file)
        
        # Sort by date (newest first)
        memo_files.sort(key=lambda p: (p.parent.parent.name, p.parent.name, p.stem), reverse=True)
    ...
    async def _process_single_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if file_key in self._file_hashes and self._file_hashes[file_key] == current_hash:
            ...
            for i, chunk in enumerate(chunks):
                ...
                metadata.content_preview = chunk[:200] + "..." if len(chunk) > 200 else chunk
    ...
    def _preprocess_content(self, content: str) -> str:
        ...
        content = re.sub(r'^#{4,}', '###', content, flags=re.MULTILINE)  # Limit header depth
    ...
    async def index_single_document(self, file_path: Path, force_reindex: bool = False) -> bool:
        """
...
class TextProcessor:
    ...
    def enhance_content_for_search(content: str) -> str:
        ...
        for abbrev, expansion in abbreviations.items():
            enhanced = re.sub(rf'\b{re.escape(abbrev)}\b', expansion, enhanced, flags=re.IGNORECASE)
...
class FileWatcher:
    ...
    async def start_watching(self) -> None:
        ...
        class MemoEventHandler(FileSystemEventHandler):
            ...
            def _handle_file_change(self, file_path):
                ...
                if path.suffix in self.indexer.config.supported_extensions:
                    asyncio.create_task(self.indexer.index_single_document(path, force_reindex=True))
            
            async def _handle_file_deletion(self, file_path):
                try:
                    await self.indexer.vector_store.remove_document(file_path)
                    self.logger.info(f"Removed deleted document from index: {file_path}")
                except Exception as e:
        ...
        self._observer.schedule(
            event_handler,
            str(self.indexer.config.data_root),
            recursive=True
        )
