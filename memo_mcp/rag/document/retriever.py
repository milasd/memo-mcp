...
class DocumentRetriever:
    ...
    def _expand_date_expressions(self, query: str) -> str:
        ...
        replacements = {
            ...
            r'\byesterday\b': (today.replace(day=today.day-1) if today.day > 1 else today).isoformat(),
            ...
            r'\blast month\b': f"{today.year}-{today.month-1:02d}" if today.month > 1 else f"{today.year-1}-12"
        }
    ...
    def _rerank_results(
        ...
        for result in results:
            ...
            position_score = self._calculate_position_score(metadata.chunk_index, metadata.total_chunks)
    ...
    def _enhance_results(self, results: List[Dict[str, Any]], query: str) -> List[Dict[str, Any]]:
        ...
        for result in results:
            ...
            enhanced_result["relevance_explanation"] = self._generate_relevance_explanation(result)
            
            # Add document summary if this is the first chunk
            if result["metadata"].chunk_index == 0:
                enhanced_result["is_document_start"] = True
                enhanced_result["document_summary"] = self._generate_document_summary(result["text"])
            else:
...
class ResultAggregator:
    ...
    def merge_results(
        ...
        for result in semantic_results:
            ...
            if key not in all_results:
                ...
                all_results[key]["similarity_score"] = max(
                    all_results[key]["similarity_score"],
        ...
        merged.sort(key=lambda x: x.get("combined_score", x["similarity_score"]), reverse=True)
